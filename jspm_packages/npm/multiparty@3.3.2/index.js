...
(function(Buffer, process) {
  ...
  Form.prototype._write = function(buffer, encoding, cb) {
    ...
    for (i = 0; i < len; i++) {
      ...
      switch (state) {
        ...
        case CLOSE_BOUNDARY:
          if (c !== HYPHEN)
            return self.handleError(new Error("Expected HYPHEN Received " + c));
          if (index === 1) {
            self.onParsePartEnd();
            state = END;
          } else if (index > 1) {
  ...
  function handleFile(self, fileStream) {
    ...
    counter.on('progress', function() {
      ...
      if (self.totalFileSize > self.maxFilesSize) {
        if (hashWorkaroundStream)
          fileStream.unpipe(hashWorkaroundStream);
  ...
  function setUpParser(self, boundary) {
    ...
    self.on('finish', function() {
      if ((self.state === HEADER_FIELD_START && self.index === 0) || (self.state === PART_DATA && self.index === self.boundary.length)) {
        self.onParsePartEnd();
